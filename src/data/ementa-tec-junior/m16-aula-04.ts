...
Para o jogo saber em qual fase a criança está, vamos usar uma variável global chamada "nível". Uma variável é como uma caixinha que guarda um número (as crianças já viram isso na Aula 3). Quando o jogo começa, a caixinha "nível" guarda o número 1. Quando o herói toca a bandeira, somamos 1 na caixinha (ela vira 2) e mandamos o Construct 3 ir para o outro Layout. Assim os dois níveis ficam conectados, como degraus de uma escada.
...
Conceito central: uma variável global chamada "nível" guarda em qual fase o jogador está. Ao tocar a bandeira, somamos 1 nessa variável e usamos a ação "Go to layout" (Ir para o layout) para trocar de cenário.
...
Conteúdo novo guiado (15 min): No seu projeto, no painel "Project" à direita, clique com o botão direito na pasta "Layouts" e escolha "Add layout" (Adicionar layout). Aparece o "Layout 2" e a "Event sheet 2". Renomeie para "Nivel2" clicando duas vezes no nome. Agora crie a variável: abra a "Event sheet" do Nível 1, clique com o botão direito numa área vazia, escolha "Add global variable" (Adicionar variável global). No nome digite "nível", tipo "Number" (Número), valor inicial "1". Confirme em "OK". Em seguida, mostre o evento da bandeira: clique em "Add event" (Adicionar evento), escolha o objeto "Heroi", a condição "On collision with another object" (Ao colidir com outro objeto) e selecione "Bandeira". Depois clique em "Add action" (Adicionar ação): escolha "System" (Sistema), ação "Add to" e selecione a variável "nível", valor "1". Adicione uma segunda ação: "System" → "Go to layout" (Ir para o layout) → escolha "Nivel2". Teste apertando o botão "Run" (Play, triângulo no topo).

Mão na massa (25 min): Cada criança faz no próprio projeto. Vá mesa a mesa. Eles vão: 1) adicionar o Layout 2; 2) desenhar nele algumas plataformas e uma bandeira (podem copiar e colar do Nível 1); 3) criar a variável "nível"; 4) montar o evento da colisão com a bandeira; 5) testar correndo até a bandeira. Quem terminar, deixa o Nível 2 mais difícil ou diferente.
...
A criança esquece de colocar uma bandeira NO Layout 2 e o jogo "trava" no segundo nível — lembre que cada nível precisa do seu chão e do seu herói. O evento aponta para o layout errado: confira em "Go to layout" se está escrito "Nivel2". O herói atravessa a bandeira sem reagir: verifique se a condição é "On collision" e se a bandeira foi mesmo selecionada. Esqueceram de criar a variável: o "Add to nível" não aparece — volte e crie a variável global primeiro. Por fim, alguns clicam em "Run" e nada acontece porque o objeto Heroi não está no Layout: arraste o Heroi para dentro do Nível 2 também.`,
  exercicios: [
    ...
    },
    {
      ...
      tempo: `7 minutos`,
      guiaProfessor: `Abra a "Event sheet" do Nível 1. Mostre o clique direito numa área vazia e "Add global variable". Reforce que o tipo é "Number" (Número) e o valor inicial é 1. Use o pote etiquetado "nível" para representar a caixinha enquanto explica. Confira nas telas se todos digitaram o nome exatamente "nível".`,
      atividade: `Abra a folha de eventos (Event sheet) do Nível 1. Clique com o botão direito num espaço vazio e escolha "Add global variable". No nome, escreva "nível". No tipo, deixe "Number". No valor inicial, coloque 1. Clique em "OK". Você criou a caixinha mágica do nível!`,
      gabarito: `O aluno deve criar uma variável global de nome "nível", do tipo "Number" (Número), com valor inicial 1. O caminho é: botão direito na folha de eventos → "Add global variable" → preencher nome, tipo e valor → "OK". A variável deve aparecer no topo da folha de eventos. Erro comum: deixar o tipo como "Text"; o correto é "Number".`,
    },
    {
      titulo: `Tocou a bandeira, passou de fase!`,
      tipo: `Montagem de evento (lógica do jogo)`,
      tempo: `10 minutos`,
      guiaProfessor: `Este é o coração da aula. Garanta primeiro que o aluno tem uma bandeira no Nível 1 e plataformas, herói e uma bandeira no Nivel2. Guie a criação do evento: "Add event" → objeto "Heroi" → condição "On collision with another object" → "Bandeira". Depois "Add action" → "System" → "Add to" → variável "nível" valor 1; e uma segunda ação "System" → "Go to layout" → "Nivel2". Mande testar no botão "Run".`,
      atividade: `Na folha de eventos do Nível 1: clique em "Add event", escolha "Heroi", depois "On collision with another object" e selecione "Bandeira". Agora clique em "Add action": escolha "System", depois "Add to", escolha a variável "nível" e digite 1. Adicione outra ação: "System" e "Go to layout", escolhendo "Nivel2". Aperte "Run" e corra até a bandeira!`,
      gabarito: `O evento correto é: Condição — Heroi → "On collision with another object" → Bandeira. Ações — 1) System → "Add to" → nível + 1; 2) System → "Go to layout" → Nivel2. Ao testar com "Run", o herói deve tocar a bandeira e o jogo deve trocar para o Nivel2, com a variável "nível" valendo 2. Se não funcionar, verificar: a bandeira foi selecionada na colisão? O "Go to layout" aponta para "Nivel2"? Existe um Heroi dentro do Nivel2?`,
    },
    {
