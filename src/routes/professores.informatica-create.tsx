...
import { ArrowLeft, Lock, Clock, BookOpen, Target, Cpu, Trophy, CircleCheck } from "lucide-react";
import { Reveal } from "@/components/reveal";
import { MesEmentaView } from "@/components/aula-detalhada";
import { ANOS_INF_CREATE } from "@/data/ementa-inf-create";
import { noindexMeta } from "@/lib/seo";

export const Route = createFileRoute("/professores/informatica-create")({
  component: MaterialInfCreate,
  head: () =>
    noindexMeta({
      title: "Informática Create — Material do Professor (restrito) | Santos Tech",
    }),
});
...
function MaterialInfCreate() {
  const [anoIdx, setAnoIdx] = useState(0);
  const [mesIdx, setMesIdx] = useState(0);
  const ano = ANOS_INF_CREATE[anoIdx];
      ...
      <header className="relative overflow-hidden text-white" style={{ background: BLUE_GRAD }}>
        <div className="mx-auto max-w-5xl px-5 py-12 sm:py-16">
          ...
          <h1 className="mt-4 text-3xl font-black leading-tight sm:text-4xl">
            Informática Create — Material do Professor
          </h1>
          <p className="mt-2 inline-flex items-center gap-2 text-[15px] font-bold text-white/90">
            <Cpu className="h-4 w-4" /> {ano.titulo} · {ano.foco}
          </p>
          <p className="mt-3 max-w-2xl text-[15px] leading-relaxed text-white/90">
            Ementa detalhada aula por aula (1 hora cada, 2 aulas por semana) — 2 anos de informática,
            IA, Office, automação e dados. Cada aula traz descrição, treinamento passo a passo para o
            professor e 5 atividades com gabarito.
          </p>
